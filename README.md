...
Targeting high-tech manufacturing, Marine, Offshore, and EPCI sectors, this solution implements a decoupled, event-driven microservices architecture to ensure high fault tolerance and real-time data visibility.
...
`[Heavy Machinery / Modbus]` ➔ `[Edge Gateway]` ➔ `[MQTT Cloud Broker]` ➔ `[Data Logger]` ➔ `[MySQL]` ➔ `[REST API]` ➔ `[Vue.js Dashboard]`
...
4. **`PcmsDashboard/` (Frontend UI)**
   - A responsive, dark-mode SCADA-like web dashboard built with Vue.js 3, Tailwind CSS, and Chart.js.
   - Features real-time live charts and equipment status indicators.
...
- **Real-Time Visualization:** Zero-refresh dashboard with automated polling.
